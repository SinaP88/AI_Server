...
def ser_to_ply_single(img, ver_lst, tri, height, wfp, reverse=True):
    ...
    for i, ver in enumerate(ver_lst):
        ...
        with open(wfp_new, 'w') as f:
            ...
            for i in range(n_vertex):
                x, y, z = ver[:, i]
                if reverse:
                    f.write(f'{x:.2f} {height-y:.2f} {z:.2f} {colors[i, 2]} {colors[i, 1]} {colors[i, 0]}\n')
                else:
                    f.write(f'{x:.2f} {y:.2f} {z:.2f} {colors[i, 2]} {colors[i, 1]} {colors[i, 0]}\n')
...
def ser_to_ply_multiple(img, ver_lst, tri, height, reverse=True):
    ...
    for i in range(n_ply):
        ...
        for j in range(n_vertex):
            x, y, z = ver[:, j]
            if reverse:
                fp.write(f'{x:.2f} {height - y:.2f} {z:.2f} {colors[j, 2]} {colors[j, 1]} {colors[j, 0]}\n')
            else:
                fp.write(f'{x:.2f} {y:.2f} {z:.2f} {colors[j, 2]} {colors[j, 1]} {colors[j, 0]}\n')
    for i in range(n_ply):
        offset = i * n_vertex
        for j in range(n_face):
            idx1, idx2, idx3 = tri[j]  # m x 3
            if reverse:
                fp.write(f'3 {idx3 + offset} {idx2 + offset} {idx1 + offset}\n')
            else:
                fp.write(f'3 {idx1 + offset} {idx2 + offset} {idx3 + offset}\n')
